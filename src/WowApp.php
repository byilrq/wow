...
final class WowApp
{
    ...
    private function verifyCaptcha(): void
    {
        ...
        if ($type === 0) {
            $answer = strtolower(trim((string)($_POST['image_captcha'] ?? '')));
            if ($answer === '' || $answer !== strtolower((string)($_SESSION['image_captcha'] ?? ''))) {
    ...
    private function pageHome(): string
    {
        ...
        <section class="hero">
            <div>
                ...
                <p>注册账号、查看服务器状态、阅读最新公告。客户端 Realmlist：<code>set realmlist <?= $this->h($this->cfg('realmlist')) ?></code></p>
                <div class="actions">
    ...
    private function captchaWidget(): string
    {
        ...
        if ($type === 0) {
            ...
            return '<label>图形验证码：' . $a . ' + ' . $b . ' = <input name="image_captcha" required inputmode="numeric"></label>';
        }
        if ($type === 1 && $key !== '') {
            return '<script src="https://js.hcaptcha.com/1/api.js?hl=' . $this->h($lang) . '" async defer></script><div class="captcha-box"><div class="h-captcha" data-sitekey="' . $this->h($key) . '"></div></div>';
        }
        if ($type === 2 && $key !== '') {
            return '<script src="https://www.google.com/recaptcha/api.js?hl=' . $this->h($lang) . '" async defer></script><div class="captcha-box"><div class="g-recaptcha" data-sitekey="' . $this->h($key) . '"></div></div>';
        }
        return '';
    }
    ...
    private function render(string $content, string $page): void
    {
        ...
        $nav = ['home' => '首页', 'register' => '注册', 'status' => '状态', 'announcements' => '公告'];
        ?>
